...
def summarize_conversation(person_prompt, conversation, verbose=False):
    ...
    messages = (
        ...
        + [
            SystemMessage(
                content=f"Now that you've had a conversation with another person, think about how you would feel if you were dating them. Summarize how compatible you feel they would be as friends, both in free-form text, and as a list of pros and cons.\n\n{conversation_parser.get_format_instructions()}"
            )
        ]
    )

    chain = (
        ChatPromptTemplate.from_messages(messages) | ChatOpenAI() | conversation_parser
    )
...
def evaluate_compatibility(person_prompt, conversation_list, verbose=False):
    ...
    summaries = [
        summarize_conversation(person_prompt, conversation, verbose=verbose)
